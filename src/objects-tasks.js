...
function mergeObjects(objects) {
  ...
  objects.forEach((element) => {
    Object.entries(element).forEach(([elem, ind]) => {
      if (!obj[elem]) {
        obj[elem] = ind;
      } else {
        obj[elem] += ind;
...
function makeWord(lettersObject) {
  const arr = [];
  const lengthWord = Object.values(lettersObject).flat().length;
  for (let i = 0; i < lengthWord; i += 1) {
    Object.entries(lettersObject).forEach(([elem, ind]) => {
      ind.forEach((el) => {
        if (el === i) {
          arr.push(elem);
        }
...
function fromJSON(proto, json) {
  ...
  Object.setPrototypeOf(newJson, proto);
  return newJson;
}
